...
import { HuggingFaceService, Intent } from "../src/huggingface/huggingface.service";
...
describe("IntentService", () => {
  it.each([
    ...
    const service = new IntentService(huggingFace);

    await expect(service.classify(textMessage(text))).resolves.toBe(expected);
    expect(huggingFace.classifyIntent).toHaveBeenCalledWith(text);
  });

  it("always captures media without spending an intent call", async () => {
    const huggingFace = { classifyIntent: jest.fn() } as unknown as HuggingFaceService;
    const service = new IntentService(huggingFace);
    ...
    expect(huggingFace.classifyIntent).not.toHaveBeenCalled();
  });
});
